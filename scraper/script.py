from appsAuth import SpotifyOperation
...
def detailsFromTracks(allTracks, counter=1, listx=[]):
    ...
    for track_id in allTracks:
        merged3 = sptfyOp.track(track_id)
        #merged2 = {**merged3, **sptfyOp.artist(merged3)} 
        merged = {**merged3, **sptfyOp.audio(track_id)}
        # add more features to dictionary
        merged['added_at'] = date_scraped
        merged['lyrics'] = SpotifyOperation.api4ovh(merged)
        listx.append(merged)
        print(f'{1} scraped with and progress is {counter}')
        if counter%10 == 0:
            time.sleep(15)
        counter += 1
    return listx
...
allArtists = (
    '5yOvAmpIR7hVxiS6Ls5DPO', '2A9kNdL3S9DKEIjhZJZZfM', '4Xj0nxVO4r7PLEaw7LRiBa', '5N9rf2O5ASaO5oiVh5is5n', '3LOm0AZjpwVQebvkyanjDy')
